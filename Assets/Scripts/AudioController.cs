...
Used to control all sound in the game to avoid having AudioSorces on individual GameObjects.

To play sound: AudioController.instance.Play(clip:AudioClip, volume:float, pitch:float);
To play music: AudioController.instance.PlayMusic(clip:AudioClip, volume:float, pitch:float, fade:boolean);
To stop music: AudioController.instance.StopMusic(fade:boolean);
...
{
	//Place audio clips (Optional: audio clips can also be played directly from gameObjects)
	public AudioClip[] _audioClips;		
	//How many audio sources can play at the same time
	public int _audioChannels = 10;		
	//Volume applied to all sound (keep at aprox 0.5 for better individual object volume control - Example: Setting object volume to 2 on events that are important)
	public float _masterVol = .5f;			
	//Volume multiplier for sound effects
	public float _soundVol = 1.0f;			
	//Volume multiplier for music
	public float _musicVol = 1.0f;		
	//Enable to change rollOff
	public bool _linearRollOff;			
	//List of audio sources (channels)
	public AudioSource[] channels;		
	//Current channel
	public int channel;					
	//List of music audio sources (channels)
	public AudioSource[] _musicChannels;	
	//Current channel
	public int _musicChannel;	
	//Cache for the music clips volume, makes controller able to change volume on runtime. Run UpdateMusicVolume(); after changing _musicVol
	float _currentMusicVol;		
	//Music will fade to this value in FadeUpMusic()
	float _fadeTo;				
	// SoundController is a singleton. SoundController.instance.DoSomeThing();
